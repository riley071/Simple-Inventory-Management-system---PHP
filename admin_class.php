<?php
// Start a session and set error reporting to display errors
session_start();
ini_set('display_errors', 1);
Class Action {
	private $db;
// Define the Action class, which handles various actions requested by users
// Constructor function to initialize the database connection
	public function __construct() {
		ob_start();
   	include 'db_connect.php';
   // Destructor function to close the database connection and flush output buffering
       $this->db = $conn;
	}
	function __destruct() {
	    $this->db->close();
...
// Query the database to check user credentials
	function login(){
		...
		if($qry->num_rows > 0){
			...
			return 3; // Return error code if login fails
		}
	}
...
// Other methods for handling various actions such as user registration, settings update, category management, product management, etc.
